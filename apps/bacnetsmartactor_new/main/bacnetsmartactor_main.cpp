...
//constexpr gpio_num_t BACNET_LED1 = GPIO_NUM_2;
constexpr gpio_num_t BACNET_LED1 = GPIO_NUM_32;
constexpr gpio_num_t BACNET_LED2 = GPIO_NUM_33;
constexpr gpio_num_t SDA_PIN = GPIO_NUM_13;
constexpr gpio_num_t SCL_PIN = GPIO_NUM_16;
        ...
        if (len)
        {
            ...
            case 'L':
                if (sscanf(msg, "L%i", &value))
                {
                    ...
                    case 0:
                        gpio_set_level(BACNET_LED1, 0);
                        processImage.led1=false;
                        break;
                    case 1:
                        gpio_set_level(BACNET_LED1, 1);
                        processImage.led1=true;
                        break;
                    case 2:
                        gpio_set_level(BACNET_LED2, 0);
                        processImage.led2=false;
                        break;
                    case 3:
                        gpio_set_level(BACNET_LED2, 1);
                        processImage.led2=true;
                        break;
                    }
...
static httpserver httpd(80, BACNET_LED1, websocket_callback);
...
{
    bme280_reading_data dummySensorData;
    dummySensorData.humidity = 50.0;
    dummySensorData.temperature = 21.0;
    dummySensorData.pressure = 1000.0;

    char jsonbuffer[512];
    BME280 bme280 = BME280();
    bme280.setDebug(true);
    bool bmeAvail = bme280.init(SDA_PIN, SCL_PIN, BME280_ADDR) == ESP_OK;
    cJSON *root, *info, *d;
    while (1)
    {
        bme280_reading_data sensor_data = bmeAvail ? bme280.readSensorData() : dummySensorData;
        ...
        cJSON_AddBoolToObject(d, "led1", processImage.led1);
        cJSON_AddBoolToObject(d, "led2", processImage.led2);
        ...
        if (!bmeAvail)
        {
            dummySensorData.temperature += 1;
            if (dummySensorData.temperature > 30)
            {
                dummySensorData.temperature = 20;
            }
        }
...
{
    gpio_pad_select_gpio(BACNET_LED1);
    gpio_set_direction(BACNET_LED1, GPIO_MODE_OUTPUT);
    gpio_set_level(BACNET_LED1, 0);

    gpio_pad_select_gpio(BACNET_LED2);
    gpio_set_direction(BACNET_LED2, GPIO_MODE_OUTPUT);
    gpio_set_level(BACNET_LED2, 0);
